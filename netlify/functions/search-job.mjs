...
export default async request=>{
  ...
  try{
    ...
    const id=new URL(request.url).searchParams.get('id');if(!id)throw new Error('Missing job id.');
    const rows=await supabaseRequest(`search_jobs?id=eq.${encodeURIComponent(id)}&select=*&limit=1`);
    if(!rows.length)return corsJson({ok:false,error:'Search job not found.'},404);
